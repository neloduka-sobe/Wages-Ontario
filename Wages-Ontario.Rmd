...
## How has the average hourly wage rate changed over the years across different age groups?
```{R}
avg_wage_by_age <- wages %>% filter(Wages == "Average hourly wage rate") %>% select(YEAR, Age.group, Both.Sexes) %>% group_by(YEAR, Age.group) %>% summarise(Avg_Hourly_Wage = mean(Both.Sexes))
avg_wage_by_age
avg_wage_by_age <- avg_wage_by_age %>%
...
```{R}
avg_wage_by_education <- wages %>% filter(Wages == "Average hourly wage rate") %>% select(Education.level, Male, Female)
avg_wage_by_education <- avg_wage_by_education %>% group_by(Education.level) %>% summarise(male_avg = mean(Male), female_avg = mean(Female))
kable(avg_wage_by_education)
```

```{R}
ggplot(avg_wage_by_education, aes(x = Education.level)) +
  geom_bar(aes(y = male_avg), stat = "identity", fill = "blue", alpha = 0.5, position = "dodge") +
  geom_bar(aes(y = female_avg), stat = "identity", fill = "red", alpha = 0.5, position = "dodge") +
